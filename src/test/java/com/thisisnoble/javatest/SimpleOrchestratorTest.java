...
public class SimpleOrchestratorTest {
    ...
    @Test
    public void MultipleTradeEventsShouldTriggerAllProcessors() {
        TestPublisher testPublisher = new TestPublisher();
        Orchestrator orchestrator = setupOrchestrator();
        orchestrator.setup(testPublisher);
        ...
        TradeEvent te = new TradeEvent(tradeEventId(), 1000.0);
        TradeEvent te2 = new TradeEvent(tradeEventId()+"2", 1000.0);
        TradeEvent te3 = new TradeEvent(tradeEventId()+"3", 1000.0);
        TradeEvent te4 = new TradeEvent(tradeEventId()+"4", 1000.0);
        TradeEvent te5 = new TradeEvent(tradeEventId()+"5", 1000.0);

        orchestrator.receive(te);
        orchestrator.receive(te2);
        orchestrator.receive(te3);
        orchestrator.receive(te4);
        orchestrator.receive(te5);
        safeSleep(100);
        CompositeEvent ce = (CompositeEvent) testPublisher.getLastEvent();
        //assertEquals(te, ce.getParent());
        assertNotNull(ce);
        assertEquals(5, ce.size());
        String lastId=ce.getParent().getId();
        RiskEvent re1 = ce.getChildById(lastId+"-riskEvt");
        assertNotNull(re1);
        assertEquals(50.0, re1.getRiskValue(), 0.01);
        MarginEvent me1 = ce.getChildById(lastId+"-marginEvt");
        assertNotNull(me1);
        assertEquals(10.0, me1.getMargin(), 0.01);
        ShippingEvent se1 = ce.getChildById(lastId+"-shipEvt");
        assertNotNull(se1);
        assertEquals(200.0, se1.getShippingCost(), 0.01);
        RiskEvent re2 = ce.getChildById(lastId+"-shipEvt-riskEvt");
        assertNotNull(re2);
        assertEquals(10.0, re2.getRiskValue(), 0.01);
        MarginEvent me2 = ce.getChildById(lastId+"-shipEvt-marginEvt");
        assertNotNull(me2);
        assertEquals(2.0, me2.getMargin(), 0.01);
    }
