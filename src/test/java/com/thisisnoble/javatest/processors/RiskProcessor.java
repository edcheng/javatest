...
public class RiskProcessor extends AbstractProcessor {
    ...
    protected Event processInternal(Event event) {
        ...
        if (event instanceof TradeEvent)
            return new RiskEvent(riskEventId(parId), parId, calculateTradeRisk(event),true);
        else if (event instanceof ShippingEvent)
            return new RiskEvent(riskEventId(parId), parId, calculateShippingRisk(event),true);
