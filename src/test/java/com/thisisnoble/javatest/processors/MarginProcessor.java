...
public class MarginProcessor extends AbstractProcessor {
    ...
    protected Event processInternal(Event event) {
        ...
        if (event instanceof TradeEvent)
            return new MarginEvent(marginEventId(parId), parId, calculateTradeMargin(event),true);
        else if (event instanceof ShippingEvent)
            return new MarginEvent(marginEventId(parId), parId, calculateShippingMargin(event),true);
